...
router.get("/fav", async (req, res) => {
  ...
  for (const q of isbns) {
    const data = (
      await api.googleBooks.get("volumes", { params: { q: `isbn:${q}` } })
    ).data;
    if (!data) {
...
router.post("/fav", async (req, res) => {
  ...
  if (!newFavs) newFavs = [];
  if (!newFavs.includes(req.body.isbn)) newFavs.push(req.body.isbn);
  else return res.status(409).json({ error: "book already a favourite" });
  ...
  try {
    await firebase
      ...
      .collection("users")
      .doc(sessionUser.uid)
      .set({ favourites: newFavs }, { merge: true });
...
router.delete("/fav", async (req, res) => {
  ...
  } else return res.status(404).json({ error: "book not a favourite" });
  try {
    await firebase
      .firestore()
      .collection("users")
